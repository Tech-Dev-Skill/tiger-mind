  ...
  return (
    <form
      action={async (formData) => {
        // Evitar que se envíe null como end_date
        const status = formData.get('subscriptionStatus') as string;
        if (status === 'pending') {
          formData.set('endDate', subscription?.end_date || new Date().toISOString());
        }
        await formAction(formData);
      }}
      className="space-y-6 bg-gray-800 rounded-lg p-8"
    >
      {/* ID del usuario */}
      ...
      <div>
        ...
        <select
          ...
          required
          className="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:ring-orange-500 focus:border-orange-500"
        >
      ...
      <div className="border-t border-gray-700 pt-6">
        ...
        <div>
          ...
          <select
            ...
            defaultValue={subscription?.status || 'pending'}
            required
            className="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:ring-orange-500 focus:border-orange-500"
          >
            <option value="pending">Inactiva</option>
