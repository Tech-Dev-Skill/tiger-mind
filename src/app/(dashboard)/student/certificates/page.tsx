...
export default async function StudentCertificatesPage() {
  ...
  const { profile, subscription, certificates } = await getStudentData(user.id);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Tus logros y certificados obtenidos
              </p>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{certificates.length > 0 ? '100%' : '0%'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">Certificados emitidos</h2>
          </div>
          
          {certificates.length > 0 ? (
            <div className="divide-y divide-gray-200">
              {certificates.map((certificate) => (
                <div key={certificate.id} className="px-6 py-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-medium text-gray-900">
                        {certificate.courses?.title || 'Curso sin título'}
                      </h3>
                      <p className="text-sm text-gray-500">
                        Certificado #{certificate.certificate_number}
                      </p>
                      <p className="text-sm text-gray-500">
                        Emitido el {new Date(certificate.issue_date).toLocaleDateString('es-ES')}
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Link
                        href={`/student/courses/${certificate.courses?.slug}`}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">No tienes certificados aún</h3>
