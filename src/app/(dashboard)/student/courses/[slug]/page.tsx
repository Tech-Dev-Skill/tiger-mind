...
export default async function StudentCoursePage({ params }: Props) {
  ...
  const { course, modules, videosWithoutModule, hasActiveSubscription, progress } = data;

  // Calcular estadísticas
  const totalVideos = modules.reduce((acc, module) => acc + (module.videos?.length || 0), 0) + videosWithoutModule.length;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-6">
              ...
              {/* Videos sin módulo asignado */}
              {videosWithoutModule && videosWithoutModule.length > 0 && (
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="p-6 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900">
                      Videos del curso
                    </h3>
                    <p className="mt-1 text-sm text-gray-600">Videos adicionales del curso</p>
                  </div>

                  <div className="divide-y divide-gray-200">
                    {videosWithoutModule.map((video: any, videoIndex: number) => {
                      const isCompleted = progress.completed_videos?.includes(video.id);
                      const canAccess = hasActiveSubscription || video.is_free_preview;
